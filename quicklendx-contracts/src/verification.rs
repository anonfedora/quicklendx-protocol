use soroban_sdk::{contracttype, Address, Env, String, symbol_short, Vec, vec, Symbol};
...
impl BusinessVerificationStorage {
    ...
    pub fn store_verification(env: &Env, verification: &BusinessVerification) {
        env.storage().instance().set(&verification.business, verification);
    ...
    pub fn get_verified_businesses(env: &Env) -> Vec<Address> {
        env.storage().instance().get(&Self::VERIFIED_BUSINESSES_KEY).unwrap_or(vec![env])
    }

    pub fn get_pending_businesses(env: &Env) -> Vec<Address> {
        env.storage().instance().get(&Self::PENDING_BUSINESSES_KEY).unwrap_or(vec![env])
    }

    pub fn get_rejected_businesses(env: &Env) -> Vec<Address> {
        env.storage().instance().get(&Self::REJECTED_BUSINESSES_KEY).unwrap_or(vec![env])
    }
    ...
    fn add_to_verified_businesses(env: &Env, business: &Address) {
        ...
        env.storage().instance().set(&Self::VERIFIED_BUSINESSES_KEY, &verified);
    }
    ...
    fn add_to_pending_businesses(env: &Env, business: &Address) {
        ...
        env.storage().instance().set(&Self::PENDING_BUSINESSES_KEY, &pending);
    }
    ...
    fn add_to_rejected_businesses(env: &Env, business: &Address) {
        ...
        env.storage().instance().set(&Self::REJECTED_BUSINESSES_KEY, &rejected);
    }
    ...
    fn remove_from_verified_businesses(env: &Env, business: &Address) {
        ...
        env.storage().instance().set(&Self::VERIFIED_BUSINESSES_KEY, &new_verified);
    }
    ...
    fn remove_from_pending_businesses(env: &Env, business: &Address) {
        ...
        env.storage().instance().set(&Self::PENDING_BUSINESSES_KEY, &new_pending);
    }
    ...
    fn remove_from_rejected_businesses(env: &Env, business: &Address) {
        ...
        env.storage().instance().set(&Self::REJECTED_BUSINESSES_KEY, &new_rejected);
    }
...
pub fn submit_kyc_application(
    ...
    if let Some(existing_verification) = BusinessVerificationStorage::get_verification(env, business) {
...
pub fn require_business_verification(
    env: &Env,
    business: &Address,
) -> Result<(), QuickLendXError> {
